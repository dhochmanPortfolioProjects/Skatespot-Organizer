...
/**
 * The Room database class for the Spot SQLite table.
 *
 * Room is a database layer on top of an SQLite database that takes care of mundane tasks that you
 * used to handle with an SQLiteOpenHelper. RoomDatabase is a database holder that serves as an
 * access point to the underlying SQLite database. The Room database uses the DAO to issue queries
 * to the SQLite database. Your Room class must be abstract and extend RoomDatabase. Usually, you
 * only need one instance of the Room database for the whole app.
 *
        ...
        if (sSpotRoomDatabase == null) { // If no instance of the database exists yet, create it and return it
            synchronized (SpotRoomDatabase.class) {
                if (sSpotRoomDatabase == null) {
                    ...
                    sSpotRoomDatabase = Room.databaseBuilder(context.getApplicationContext(),
